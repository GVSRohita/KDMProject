...
# reading abstracts from Abstracts folder
new_list = []
for root, dirs, files in os.walk("/Users/GowthamYesG/Desktop/KDM/Project/Abstracts"):
    print(files)

# Dividing the whole set of abstracts into 3 sections
def chunks(lst, n):
...
for file_list in chunked_list:
    ...
    # Cleaning data - converting list to a string
    ...
    stemming = [stemmer.stem(str(e)) for e in
                clean_data_word]  # word in clean_data_word is a char array and converting that to string
    ...
    stop_words.extend(
        ['news', 'say', 'use', 'not', 'would', 'say', 'could', '(', ')', ',', '%', '_', 'be', 'know', 'good', 'go',
         'get', 'do', 'took', 'time', 'year',
